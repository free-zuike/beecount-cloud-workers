...
export async function uploadToDrive(
  ...
  const token = await refreshAccessToken('drive', config.client_id!, config.client_secret!, config.refresh_token!);
...
export async function uploadToOneDrive(
  ...
  const token = await refreshAccessToken('onedrive', config.client_id!, config.client_secret!, config.refresh_token!);
  if (!token) return false;
...
export async function uploadToDropbox(
  ...
  const token = await refreshAccessToken('dropbox', config.client_id!, config.client_secret!, config.refresh_token!);
  if (!token) return false;
