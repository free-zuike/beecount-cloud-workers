  ...
  c.executionCtx.waitUntil((async () => {
    ...
    try {
      ...
      if (!backupPath) {
        // 从 R2 查找最新备份
        if (c.env.R2) {
          const listing = await c.env.R2.list({ prefix: `backups/${schedule.user_id}/` });
      ...
      console.debug(`[Restore] Completed: ${result.message}`);
    } catch (err) {
