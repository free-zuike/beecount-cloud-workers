...
async function resolveLedger(db: D1Database, userId: string, ledgerId?: string | null): Promise<{ id: string; external_id: string; name: string } | null> {
  if (ledgerId) {
    return db.prepare(`SELECT l.id, l.external_id, l.name FROM ledgers l LEFT JOIN ledger_members lm ON l.id = lm.ledger_id WHERE (l.user_id = ? OR lm.user_id = ?) AND l.external_id = ?`).bind(userId, userId, ledgerId).first<{ id: string; external_id: string; name: string }>();
  }
  return db.prepare(`SELECT l.id, l.external_id, l.name FROM ledgers l LEFT JOIN ledger_members lm ON l.id = lm.ledger_id WHERE (l.user_id = ? OR lm.user_id = ?) ORDER BY l.created_at ASC LIMIT 1`).bind(userId, userId).first<{ id: string; external_id: string; name: string }>();
}
  ...
  try {
    switch (name) {
      ...
      case 'get_active_ledger': {
        ...
        r = led ? { id: led.external_id, name: led.name, currency: 'CNY' } : null;
        break;
      }
