  ...
  async upload(key: string, body: ArrayBuffer, contentType: string): Promise<boolean> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error('[S3Service] upload: failed request url:', url);
        console.error('[S3Service] upload: failed request headers:', JSON.stringify(headers));
        console.error('[S3Service] upload: failed response headers:', JSON.stringify(Object.fromEntries(response.headers)));
      }
      return response.ok;
    } catch (err) {
