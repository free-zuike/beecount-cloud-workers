...
// Import the ACTUAL sqlite-writer by compiling TypeScript
...
async function test() {
  ...
  const tables = {
    users: [
      {id:'user-1', email:'test@test.com', password_hash:'$2b$10$abcdefghijklmnop', is_admin:0, is_enabled:1, created_at:'2026-01-01', totp_secret_encrypted:null, totp_enabled:0, totp_enabled_at:null},
      {id:'user-2', email:'admin@test.com', password_hash:'$2b$10$xyzwvutsrqponmlkjihg', is_admin:1, is_enabled:1, created_at:'2026-06-15', totp_secret_encrypted:null, totp_enabled:1, totp_enabled_at:'2026-06-20'},
    ],
    ledgers: [
  ...
  try {
    ...
    for (const name of tableNames) {
      ...
      if (count > 0) {
        const data = db.exec('SELECT * FROM "' + name + '" LIMIT 1');
