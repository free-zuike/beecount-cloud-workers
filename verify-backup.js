...
async function main() {
  ...
  console.log('\n=== db.sqlite3 ===');
  console.log('  Size: ' + sqliteData.length.toLocaleString() + ' bytes');
  ...
  try {
    ...
    for (const name of tableNames.slice(0, 3)) {
      const r = db.exec('SELECT * FROM "' + name + '" LIMIT 1');
      if (r.length > 0) {
        console.log('  ' + name + ': cols=[' + r[0].columns.join(', ') + ']');
      }
    }
